...
Core 与 UI 的通知采用函数回调方式，约定如下：

1. 数据刷新通知：`ui_on_data_updated()`  
- 触发时机：Core 完成加载、过滤、编辑、删除、保存、导出后的状态变更。  
- 处理要求：UI 在该回调中统一刷新表格、详情、KPI、饼图和报告区。

2. 日志通知：`ui_push_log(const char *msg)`  
- 触发时机：Core 在关键流程节点或异常分支产生文本反馈时。  
- 数据约定：`msg` 必须为 `\0` 结尾字符串；UI 需处理空串和缓冲区上限。

3. 顺序约定：  
- 建议先写入日志再触发数据刷新，保证界面刷新后可看到对应操作说明。  
- 同一业务操作最多触发一轮“日志 + 刷新”，避免重复刷新导致闪烁。

4. 失败约定：  
- Core 返回非 `WL_OK` 时，必须至少触发一次日志通知说明失败原因。  
- UI 接收失败通知后不得直接修改 Core 内部状态，只能引导用户重试或修正输入。
